...
router.get('/', function(req, res, next) {
    ...
    var limitx = req.query.limit
    var sortx = req.query.sort
    ...
    if (filterx){
        ...
        if(filter_jumlah == null){
            var filter_ = String()
        } else if(filter_jumlah == 1){
            filter_ = "Where "+String(filter_kiri[0])+' = '+"'"+String(filter_kanan[0])+"'"
    ...
    if(limitx){
        var limit_ = " limit "+String(limitx)
    }
    else{limit_ = String()}

    if(sortx){
        var sort_ = " order by "+sortx 
    }
    else{sort_ = String()}
    ...
    'ON a.id_provinsi = f.id left join kota g ON a.id_kota = g.id WHERE b.nama_kelompok = '+String(nama_data4)+') ' + 
    'as seluruh '+filter_+' '+sort_+' '+limit_
